...
export function AdminDashboard({username}: {username: string}) {
  ...
  const csv = useMemo(() => {
    const headers = ["编号", "创建时间", "咨询方向", "单位名称", "联系人", "电话", "微信", "城市", "期望启动", "范围", "预算", "状态", "业务场景"];
    ...
    <main className="site-shell min-h-screen py-10">
      ...
      <Card className="mt-6" variant="default">
        <Card.Content>
          <div className="grid gap-3 md:grid-cols-[1fr_220px_auto]">
            <Input aria-label="搜索咨询" onChange={(event) => setQuery(event.target.value)} placeholder="搜索单位、联系人、电话、业务场景" value={query} />
      ...
      <div className="mt-6 overflow-x-auto rounded-2xl border border-[var(--border)] bg-white">
        <table className="admin-table">
          <thead><tr><th>时间 / 编号</th><th>单位与联系人</th><th>咨询方向与业务场景</th><th>预算 / 时间</th><th>状态</th></tr></thead>
          <tbody>{leads.map((lead) => <tr key={lead.id}>
            ...
            <td className="min-w-48"><strong>{lead.companyName || "未填写单位"}</strong><br />{lead.contactName} · {lead.phone}<br /><span className="text-xs text-[var(--muted)]">{lead.wechat || "无微信"} · {lead.city || "未填地区"}</span></td>
            <td className="min-w-80"><Chip color="accent">{lead.serviceType}</Chip><p className="mt-2 max-w-xl leading-6">{lead.description}</p></td>
            <td className="min-w-40">{lead.budget || "待评估"}<br /><span className="text-xs text-[var(--muted)]">{lead.expectedDate || "时间待沟通"} · {lead.scale || "范围待沟通"}</span></td>
        ...
        {!loading && !leads.length ? <p className="p-10 text-center text-[var(--muted)]">暂无符合条件的咨询</p> : null}
      </div>
    </main>
  );
}
